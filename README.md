...
- **Framing**: 4-byte length prefix + bincode payload for control messages, 2-byte port header for data streams
...
|-------|------|-------------|
| `listen_addr` | String | UDP address to listen on (e.g., `"0.0.0.0:4433"`) |

### Client Configuration

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `remote_addr` | String | Required | Server address (`"host:port"`) |
| `prefer_ipv6` | Boolean | `false` | Prefer IPv6 when resolving server address |
| `retry_interval` | Integer | `5` | Initial retry interval in seconds |
| `services` | Array | Required | List of services to expose |

### Service Definition

| Field | Type | Description |
|-------|------|-------------|
| `service_name` | String | Human-readable name for logging |
| `local_addr` | String | Local service address (`"host:port"`) |
| `remote_port` | Integer | Public port on server |
...
- **TLS**: QUIC provides built-in TLS 1.3 encryption
- **Self-Signed Certificates**: Server generates self-signed certificates by default. For production, consider using proper certificates
- **No Authentication**: Current version has no client authentication. Run on trusted networks or add authentication layer
- **Firewall**: Only expose necessary ports on the server
...
- **Compio Runtime**: Uses completion-based I/O for optimal performance
...
- [ ] **Client Authentication**: Token-based or certificate-based client authentication
- [ ] **Custom TLS Certificates**: Support for user-provided certificates instead of self-signed
- [ ] **UDP Proxying**: Support for UDP services in addition to TCP
- [ ] **Multi-Server Support**: Client connecting to multiple servers for redundancy
...
- [compio](https://github.com/compio-rs/compio) - Completion-based async runtime
- [compio-quic](https://github.com/compio-rs/compio) - QUIC implementation for compio
